...
#------------------------------------------------------------------------------
def ReadGsheetURL(sheeturl, headerline=7, usecolmns=[0,1,2,3,4,5]):
    df = pd.read_csv(sheeturl, skiprows=headerline, index_col=None, usecols=usecolmns) # 1st row comment line
    df = df.dropna(how='all').reset_index(drop=True)
    df = df.fillna('')
    df['Calender week'] = df['Calender week'].astype(int)
    WeekNumber, _ = GetDateParameters()
    df = df[df.index[df['Calender week'] == WeekNumber][0]:].reset_index(drop=True)   
    return df
...
def CheckUpdated(sheeturl, fname, payload=None, headerline=7):
    df = ReadDatabase(fname)
    df_nnew = ReadGsheetURL(sheeturl, headerline=headerline)
    if not df.equals(df_nnew):
        ...
        try:
            if not df[df.index[df['Calender week'] == WeekNumber][0]:].reset_index(drop=True).equals(df_nnew):
...
def SendMattermostNotification(df):
    ...
    payload=f'payload={{"icon_emoji":":group-image:","username": "my-group","text": "### The group seminar schedule has been updated ({todaydate}) @all\n{header}"}}'
    return payload
...
def UpdateDescription(fname, maxrepetation=2, Description=None):  
    df = ReadDatabase(fname)
    ...
    TodaysList = df.iloc[df.index[df['Calender week'] == WeekNumber]]
...
def SendWeeklyNotification(fname):
    df = ReadDatabase(fname)
    WeekNumber, _ = GetDateParameters()
    WeekPos = df.index[df['Calender week'] == WeekNumber][0]
    ...
    payload=f'payload={{"icon_emoji":":group-image:","username": "my-group","text": "### This week group seminar schedule (last updated on {local_time})\n{mytable}"}}'
    return payload
